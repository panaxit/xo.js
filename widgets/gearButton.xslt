...
>
	...
	<xsl:template mode="gearButton:widget" match="@*">
		...
		<span class="input-group-append" style="color:black;">
			...
			<xsl:variable name="command" select="$xo:context/ancestor-or-self::data:rows[1]/@command"/>
			<xsl:choose>
				...
				<xsl:when test="$command">
					<button type="button" class="btn btn-outline-secondary btn-lg dropdown-toggle h-100" data-bs-toggle="dropdown" aria-expanded="false" tabindex="-1" xo-scope="{$command/ancestor-or-self::*[1]/@xo:id}" xo-slot="command">
						...
						<menu class="dropdown-menu">
							<li>
								<a class="-toggle dropdown-item" href="#">Cancelar carga</a>
							</li>
						</menu>
					</button>
				</xsl:when>

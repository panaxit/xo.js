...
xo.listener.on(['beforeTransform::px:Entity'], function ({ event }) {
    ...
    for (let association_ref of node.select(`//px:Entity/px:Record/px:Association[@Type="belongsTo"][px:Mappings/px:Mapping[not(@Referencee=ancestor::px:Entity[1]/@IdentityKey)][2]]`)) {
        ...
        let referencers = Object.fromEntries(association_ref.select('px:Mappings/px:Mapping[not(@Referencee=ancestor::px:Association[1]/px:Entity/@IdentityKey)]/@Referencer').map(referencer => [referencer.value, referencer.parentNode.getAttribute("Referencee")]));
...
xo.listener.on(`change::xo:r/@*[not(contains(namespace-uri(),'http://panax.io/state')) and not(contains(namespace-uri(),'http://panax.io/metadata'))]`, async function ({ node, element: row, value }) {
    ...
    if (!field) return;
    let some_matches;
    for (let association of row.select(`ancestor::px:Entity[1]/px:Record/px:Association[@Type='belongsTo'][px:Mappings/px:Mapping/@Referencer="${node.name}"]`)) {
            ...
            if (row.$('self::*[@state:delete]')) {
                ...
                if (id_node ? id_node.value : primary_fields.filter(field => {
                    ...
                    fields.forEach(field => {
                        let isPK = primary_fields.some(pf => pf.value == field.value);
                        let field_node = xo.xml.createNode(`<field xmlns="http://panax.io/persistence" name="${field}"${isPK ? ` isPK="true"` : ''}/>`);
                        field_node.textContent = [row.get(field.value)].map(val => val ? val.value : (field.$("../@defaultValue") || 'null') || `'${val}'`);
                        dataRow.append(field_node);
                    })
                }
            }
